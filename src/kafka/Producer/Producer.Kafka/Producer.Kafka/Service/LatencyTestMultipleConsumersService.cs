...
{
    public class LatencyTestMultipleConsumersService : IHostedService
    {
        ...
        public LatencyTestMultipleConsumersService(int size, int messageSize)
        {
            _size = size;
        ...
        public void RunTest(byte[] data)
        {
            ...
            {
                _producer.Produce(topicName, new Message<Null, string> { Value = JsonConvert.SerializeObject(new KafkaMessage(data, false, DateTime.UtcNow.Ticks))});
            }
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            for (int i = 0; i <2   ; i++)
            {
                ...
                parts.ForEach(topicPart => _producer.Produce(topicPart, new Message<Null, string> { Value = JsonConvert.SerializeObject(new KafkaMessage(Array.Empty<byte>(), true, 0)) }));
                Task.Delay(40000, cancellationToken).Wait(cancellationToken);
            }
